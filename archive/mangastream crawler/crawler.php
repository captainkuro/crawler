...
class Crawler {
	...
	//sementara saja ini
	public function go2lineregexor($reg, $c = 0, $where = false) {
		$this->curline = fgets($this->stream);
		if (!$c) {
			while (preg_match($reg, $this->curline, $a) === 0 && strpos($this->curline, $where) === false && !feof($this->stream)) {
				$this->curline = fgets($this->stream);
			}
		} else {
			while (preg_match_all($reg, $this->curline, $a) != $c && strpos($this->curline, $where) === false && !feof($this->stream)) {
				$this->curline = fgets($this->stream);
			}
		}
	}
